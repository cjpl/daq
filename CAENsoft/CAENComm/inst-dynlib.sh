...
if [[ ${ARCH} = 'x86_64' ]]; then
    LIBFILE=${LIBPATH}/x64/libCAENComm.so.${VERSION}
else
    LIBFILE=${LIBPATH}/libCAENComm.so.${VERSION}
fi

if [[ -z "$1" ]]; then
    echo "Destination path not specified. Where do your want to put the libCAENComm.so?"
    echo -n "Input path: "
    read res
    DESTDIR=$res
else
...
echo "Copying \"CAENComm.h\" to $DESTDIR/include ..."
cp -p include/CAENComm.h $DESTDIR/include/
echo "Copying $LIBFILE to $DESTDIR ..."
cp -p $LIBFILE $DESTDIR/lib/
echo "Symbolic linking $DESTDIR/libCAENComm.so to $DESTDIR/libCAENComm.so.${VERSION}"
cd $DESTDIR/lib
ln -s libCAENComm.so.${VERSION} libCAENComm.so
