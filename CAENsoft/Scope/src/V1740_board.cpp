...
bool V1740Board::LoadConfig( wxConfigBase* p_config, const wxString& base_section)
{
	...
	*/
	...
	p_config->Read( base_section+ _("ADDRESS"), &board_add_string, _("0"));
	...
	this->m_p_data->m_base_address= (UINT16)board_add;

	//
	// Get the Channel group
	p_config->Read( base_section+ _("CHANNEL_GROUP"), &this->m_channel_group, 0);
	//
...
bool V1740Board::SaveConfig( wxConfigBase* p_config, const wxString& base_section)
{
	...
	p_config->Write( base_section+ _("CHANNEL_GROUP"), this->m_channel_group);
