...
AppSettings::AppSettings( void (* scope_refresh)( int, bool), void (*update_samples_ready_led)( bool)): m_vme_link(0), m_vme_board_num(0), m_vme_handle( CAEN_VME_INVALID_HANDLE),
								ScopeRefresh( scope_refresh), UpdateSamplesReadyLed( update_samples_ready_led), m_mutex( wxMUTEX_RECURSIVE)
{
	...
	setting_filename= AppSettings::get_default_path()+ _("settings.txt");
	if( wxTheApp->argc>= 2) {
	...
	*/
	...
	for( int scope_index= 0; scope_index< SCOPE_NUM_PANELS; scope_index++)
	{
	  wxString scope_index_string= wxString::Format(_("_%d"), scope_index);
		...
		config->Read( _("/MISC/OFFSET_SEC")+ scope_index_string, &this->m_offset_sec[ scope_index], 0.0);
	...
	for( i= 0; i< (unsigned int)acq_board_num; i++)
	{
		...
		if( !board_type.Trim().CmpNoCase( _("")))
		{
			...
			config->Read( board_string+ _("ADDRESS"), &board_add_string, _("0"));
			...
			tmp_data.m_base_address= (UINT16)board_add;
...
bool AppSettings::Save( void)
{
	...
	for( int scope_index= 0; scope_index< SCOPE_NUM_PANELS; scope_index++)
	{
	  ...
	  wxString grid_line_color_rgb_string= wxString::Format(_("0x%02x%02x%02x"), this->m_grid_line_color[ scope_index].Red(), this->m_grid_line_color[ scope_index].Green(), this->m_grid_line_color[ scope_index].Blue());
		...
		if( !config->Write( _("/MISC/OFFSET_SEC")+ scope_index_string, this->m_offset_sec[ scope_index]))
			return false;
...
bool AppSettings::InitVME( void)
{
	...
	if( !this->m_vme_board_type_string.CmpNoCase(_("V1718")))
	{
	...
	else if( !this->m_vme_board_type_string.CmpNoCase(_("V2718")))
	{
		CVErrorCodes cv_error_code;
	...
	else
	{
		...
		wxLogError( wxString::Format( _("Cannot find any vme board (V1718 or V2718) on link '%d' and board number '%d'"), this->m_vme_link, this->m_vme_board_num));
		return false;
	}
	return true;
}
